...
export const api_v1_users_list = createAsyncThunk("users/api_v1_users_list", async payload => {
  const response = await apiService.api_v1_users_list(payload);
  return response.data;
});
export const api_v1_users_retrieve = createAsyncThunk("users/api_v1_users_retrieve", async payload => {
  const response = await apiService.api_v1_users_retrieve(payload);
  return response.data;
});
export const api_v1_users_destroy = createAsyncThunk("users/api_v1_users_destroy", async payload => {
...
const usersSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(api_v1_users_list.pending, (state, action) => {
